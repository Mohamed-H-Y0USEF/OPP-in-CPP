...
void LibrarySystem::searchBooks_byPrefix() {
    string prefix;
...
int LibrarySystem::GetIdByName(string &str) {
    for (int i = 1; i <= len; ++i)
     if(books[i].name == str) return books[i].id;
}

bool LibrarySystem::sortByName(Book &b1, Book &b2) {
    return b1.name < b2.name;
}

bool LibrarySystem::sortById(Book &b1, Book &b2){
    return b1.id < b2.id;
}
...
bool LibrarySystem::read_user_name_and_book_name(int &userIdx, int &bookIdx, int trials) {
    string user_name;
    string book_name;
...
int LibrarySystem::find_user_idx_by_name(string &name) {
    for (int i = 1; i <= len ; ++i)
        if(users[i].Name == name)return i;
    return -1;
}

int LibrarySystem::find_book_idx_by_name(string &name) {
    for (int i = 1; i <= len ; ++i)
